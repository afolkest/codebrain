...
def export_cursor(db_path: Path = cursor_export.DEFAULT_CURSOR_DB,
    ...
    with archive_lock(root) as root_fd:
        state = _read_exporter_state(root)
        ...
        try:
            ...
            pending = {
                sid for sid in state.get("pendingComposerIds", [])
                if isinstance(sid, str) and sid
            } if isinstance(state.get("pendingComposerIds"), list) else set()
            if due_full:
                pending.intersection_update(ids)
            ids.update(pending)
            candidates = sorted(
                sid for sid in ids
                if due_full or sid in pending or sid in invalid_header_ids
                or tokens.get(sid) != old_tokens.get(sid)
            )
            ...
            for sid in candidates:
                ...
                except cursor_export.CursorSessionUnsettled:
                    stats["skipped"] += 1
                    new_pending.add(sid)
                    continue
                except (OSError, sqlite3.Error, cursor_export.CursorSnapshotError,
...
def _read_exporter_state(root: Path) -> dict:
